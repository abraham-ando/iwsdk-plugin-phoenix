  ...
  it('ne produisent jamais de valeur non finie, ni de personnage absurde', () => {
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      for (const bone of c.restPose) {
        ...
        // tête (0.25/0.133 ≈ 1.880), et 1 partout ailleurs. La borne serre
        // volontairement : changer LENGTH_SPAN ou une courbe doit devenir un
        // geste délibéré, pas un glissement muet.
        expect(bone.scale).toBeGreaterThan(0.25);
        expect(bone.scale).toBeLessThan(2);

        // La SEULE grandeur que le génome pilote vraiment. Le facteur appliqué
        // à la translation d'un os est stature × propre × ageFactor, chacun
        // borné : lengthFactor ∈ [0.75, 1.25] et limbRatio ∈ [0.62, 1]. Le
        // produit vit donc dans [0.349, 1.5625], et un os hors chaîne garde 1.
        const repos = rig.bones[bone.role]!;
        const normeRepos = Math.hypot(repos.position[0], repos.position[1], repos.position[2]);
        if (normeRepos > 0) {
          const facteur =
            Math.hypot(bone.position[0], bone.position[1], bone.position[2]) / normeRepos;
          expect(facteur).toBeGreaterThan(0.34);
          expect(facteur).toBeLessThan(1.57);
        }
      }

      // Tautologie aujourd'hui — l'influence est une combinaison convexe de deux
      // constantes déjà dans [-1, 1]. Gardée comme fil de détente : elle casse si
      // quelqu'un élargit une plage de morph au-delà de [-1, 1] dans humanoid.ts.
