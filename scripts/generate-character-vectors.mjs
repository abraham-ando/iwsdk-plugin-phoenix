...
const lines = [
  ...
  '# Tab-separated: seed, age, nominalHeightMeters, groundOffsetMeters, then',
  '# role=x,y,z,scale per bone, then morph=value. Floats are fixed to 6 decimals.',
  ['seed', 'age', 'height', 'groundOffset', 'bones', 'morphs'].join('\t'),
];
...
for (const seed of SEEDS) {
  ...
  for (const age of AGES) {
    ...
    const morphs = Object.keys(c.morphs)
      ...
      .join(' ');
    lines.push(
      [seed, age, f(c.stats.nominalHeightMeters), f(c.stats.groundOffsetMeters), bones, morphs].join('\t'),
    );
  }
}
