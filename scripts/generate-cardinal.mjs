...
// TypeScript source imports its siblings with a `.js` suffix; Node strips
// types but does not remap that. Registered before any dynamic import below.
...
/**
 * Frame a batch: header, then a header and payload per record.
 *
 * The payloads come from the shipped codec above; only the 7-byte frame
 * header and 6-byte record headers are assembled here. `BinaryProtocol.ts`
 * cannot be imported directly because `opcodes.ts` declares a TypeScript
 * `enum`, which Node's strip-only type support cannot erase. Duplicating two
 * fixed-width headers is the proportionate answer — the layout that is
 * actually complex, the component payload, is still produced by the code that
 * ships. Both client and server unit tests assert this same framing
 * independently.
 */
function frameComponentUpdate(records, serverTick) {
  const OP_COMPONENT_UPDATE = 12;
  const headerBytes = 7;
  const recordHeaderBytes = 6;

  const total = records.reduce(
    (sum, record) => sum + recordHeaderBytes + CARDINAL_REGISTRY.get(record.componentId).bytes,
    headerBytes,
  );

  const view = new DataView(new ArrayBuffer(total));
  view.setUint8(0, OP_COMPONENT_UPDATE);
  view.setUint16(1, records.length, true);
  view.setUint32(3, serverTick, true);

  let offset = headerBytes;
  for (const record of records) {
    const spec = CARDINAL_REGISTRY.get(record.componentId);
    view.setUint32(offset, record.networkId, true);
    view.setUint16(offset + 4, record.componentId, true);
    spec.encode(view, offset + recordHeaderBytes, record.data);
    offset += recordHeaderBytes + spec.bytes;
  }
...
// Whole-frame vectors: the record layout is proven above, this proves the
// batching around it. One empty batch, one holding a record per component.
vectorLines.push('#', '# component_update <serverTick> <componentId:networkId ...> <hex>');
...
for (const { tick, records } of frameCases) {
  const frame = frameComponentUpdate(records, tick);
  vectorLines.push(
    [
      'component_update',
      tick,
      // The values themselves are already pinned by the `cardinal` rows above;
      // what a frame vector adds is the header and the record framing, so it
      // only needs to name which record went where.
      records.map((r) => `${r.componentId}:${r.networkId}`).join(',') || '-',
