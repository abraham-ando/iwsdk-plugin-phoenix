...
describe('le modèle par défaut a une source unique', () => {
  it("NE VIT NULLE PART EN DUR : ni le worker, ni le plugin ne l'écrivent", () => {
    // Un identifiant recopié à deux endroits finit par diverger, et le worker
    // charge alors autre chose que ce que le plugin annonce. Le catalogue est
    // seul à le nommer ; sa validité face à WebLLM est vérifiée à part.
    for (const rel of ['../src/workers/llm.worker.ts', '../src/plugin.ts']) {
      const src = readFileSync(new URL(rel, import.meta.url), 'utf8');
      expect(src, rel).toContain('DEFAULT_LOCAL_MODEL');
      expect(src.match(/'[A-Za-z0-9._-]*-MLC'/), `${rel} code un identifiant en dur`).toBeNull();
    }
  });
});
